...
const tools = [
    { icon: 'mdi-select', label: 'Select', color: '#00BCD4' },
    { icon: 'mdi-format-text', label: 'Text', color: '#2196F3' },
    { icon: 'mdi-image-outline', label: 'Image', color: '#4CAF50' },
    { icon: 'mdi-table', label: 'Table', color: '#FF9800' },
    { icon: 'mdi-shape-outline', label: 'Shape', color: '#9C27B0' },
    { icon: 'mdi-vector-line', label: 'Line', color: '#795548' },
    { icon: 'mdi-view-grid-outline', label: 'Grid', color: '#607D8B' },
    { icon: 'mdi-currency-usd', label: 'Pricing', color: '#FF5722' },
    { icon: 'mdi-signature-freehand', label: 'Signature', color: '#3F51B5' },
    { icon: 'mdi-pencil', label: 'Draw', color: '#E91E63' }
]
...
<template>
  <v-app>
    <v-app-bar color="white" elevation="1" height="48">
      ...
      <v-btn color="success" class="text-none mr-2" size="small">
            ...
            <div class="vertical-toolbar">
              <v-tooltip v-for="tool in tools" :key="tool.label" :text="tool.label" location="left">
                <template v-slot:activator="{ props }">
                  <v-btn v-bind="props" icon variant="text" :color="tool.color" class="tool-button">
...
.section-sidebar {
  position: relative;
  z-index: 3; /* Increased z-index to stay on top */
...
.vertical-toolbar {
  ...
  top: 20px;
  margin-left: 16px;
  z-index: 3; /* Same z-index as sidebar to stay on top */
}
